...
int main() {
	...
	if(ftg > 0) {
		sprintf(tmp_string, "Sully_%d.cpp", ftg);
		FILE* file = fopen(tmp_string, "r");
		if(!file)
			ftg++;
		fclose(file);
		sprintf(tmp_string, "Sully_%d.cpp", ftg-1);
		file = fopen(tmp_string, "w");
		fprintf(file, generate_string(),9,10,34,generate_string(), ftg-1);
		fclose(file);
		sprintf(tmp_string, "clang++ Sully_%1$d.cpp -o Sully_%1$d", ftg-1);
		system(tmp_string);
